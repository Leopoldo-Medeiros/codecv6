...
**URLs:** API at `http://codecv6.ddev.site` (HTTPS also works but the dev TLS cert needs to be trusted by the browser), Frontend at `http://localhost:3001` (or your LAN IP on port 3001 — see Frontend section)
...
ddev artisan content:sync                                                          # Sync curriculum from database/content/ (idempotent; --dry-run supported)
...
npm run dev                    # Dev server at http://localhost:3001 / http://<LAN-IP>:3001
...
**Dev server port is 3001** (set in `frontend/nuxt.config.ts` `devServer` — 3000 was often occupied by another process). CORS (`FRONTEND_URL`) and `SANCTUM_STATEFUL_DOMAINS` in `.env` must list the port 3001 origins; `.env.example` already does. The server listens on `0.0.0.0`, so use the LAN IP (e.g. `192.168.1.39:3001`) when testing from another device — but every origin you browse from must be in the CORS list. (Production is different: the frontend container serves on 3000 behind a proxy — see `DEPLOYMENT.md`.)
...
- **Public — waitlist** (demand-sensing): `POST /public/waitlist` (`throttle:5,1` by IP) — anonymous "coming soon" signup, idempotent `updateOrCreate` unique on `(email, topic)`, topics allow-listed in `config/waitlist.php`. Logged-in variant `POST /waitlist` (`throttle:20,1`) links the vote to the account and always uses the account's own email (client-supplied email ignored). Admin readout: `GET /admin/waitlist` — signup counts per candidate track (see Waitlist below)
- **All authenticated**: Read paths (`GET /paths`, `GET /my-paths`), courses, plans, jobs; view/update own user + avatar (`/users/{user}`, ownership enforced in controller/service — role changes are admin-only, silently ignored for other callers, see Authorization below); `PATCH /me/onboarding`; `GET /me/progress` (XP/streak/badges snapshot, see Gamification below); `GET /me/coaching-recommendation` (F6 coaching upsell nudge, see below); `GET /me/activity` (daily practice counts for the dashboard contribution heatmap — derived from `UserChallengeCompletion.completed_at` + incident-step completions, no activity log); CV/LinkedIn analysis (`throttle:5,1` — Gemini + optional Jina fetch); Stripe checkout (`POST /checkout/session`, `throttle:10,1`); challenges (`GET /challenges` — each row carries `locked` (F4) and `solved` (from `UserChallengeCompletion`, one query, no N+1); `GET /challenges/{slug}`, `GET /challenges/{slug}/submissions` (the caller's own iteration history — every run is recorded in `challenge_submissions`, capped at the latest 20 per user+challenge), `POST /challenges/{slug}/run` — `throttle:20,1`, Judge0 sandbox run); playground (`POST /playground/run`, throttled 30/min); step progress (`PUT /path-steps/{step}/progress`); quiz grading (`POST /path-steps/{step}/quiz` — server-side, F4-gated, see Quizzes below); notifications (`GET /notifications`, `PATCH /notifications/{id}/read`, `PATCH /notifications/read-all`); GDPR export (`GET /users/{user}/export` — self or admin, checked in controller)
...
- **Dashboard (`pages/dashboard.vue`):** client view has a KPI row (XP/streak/steps/progress, each with a mini viz — `AreaChart`/`MiniBars`/progress bar), a GitHub-style `ActivityHeatmap` (from `GET /me/activity`), a `Practice over time` `AreaChart`, `Continue learning` cards, and the right rail (`ProgressWidget`, `CoachingNudge`, `WaitlistVote` — one-tap vote on candidate tracks, quick actions). The admin/staff view mirrors it from `GET /admin/dashboard`: KPIs (Total Users signups sparkline, Revenue-EUR area), a platform-wide `ActivityHeatmap`, a `Revenue over time` `AreaChart`, the recent-users table, and `WaitlistAdminCard` (per-track signup counts — the "what to build next" readout) in the right rail. Each `StatCard` carries its own `series`/`bars`/`ratio` so KPI mini-viz is data-driven. Dashboard chart components: `ActivityHeatmap.vue` (contribution grid), `AreaChart.vue` (fills parent height — set a height on the wrapper), `MiniBars.vue`
...
### Database Seeding & the Content Pipeline
...
4. `ContentSeeder` — thin wrapper that runs `php artisan content:sync` (all curriculum: challenges, paths, steps)
5. `PlanSeeder` — seeds training plans
6. `BadgesSeeder` — seeds the badge catalog
...
**The curriculum lives in versioned files, not seeders.** `database/content/` holds 34 challenges (`challenges/<slug>/` with `meta.json`, `description.md`, `boilerplate.php`, `tests.php`) and 8 paths (`paths/<slug>/meta.json` + `steps/NN-<slug>.md`, each step a `---json` front-matter block + `concept_content` Markdown body). `php artisan content:sync [--dry-run]` upserts by challenge slug / path name / `(path_id, order)`, preserves `created_by`/`consultant_id`, and is proven idempotent (`tests/Feature/ContentSyncTest.php`). **Editing a lesson = editing a content file + running `content:sync`** — never write curriculum in seeder code. `pint.json` excludes `database/content` so the linter never restyles authored content. The old heredoc seeders (`ChallengeSeeder`, `LearningPathsSeeder`, `PathStepSeeder`, `IncidentSeeder`, `RestApiTrackSeeder`) were deleted.
...
**Certification seals:** A `Path` may confer a badge on full completion via its nullable `paths.badge_key` column — when `GamificationService` detects every step of a path is `done`, it awards `path_completed` (generic) plus the path's `badge_key` if set. Badges carry a `category` (`achievement` vs `certification`); the public skill profile (F3) renders certifications as a prominent **seal** (distinct emerald card, "Certified by CODECV"), separate from achievement chips. The Observability track ("Observability 101", `badge_key = 'observability_certified'` in its `database/content/paths/observability-101/meta.json`) is the first certification — it's the employer-facing proof the track is designed to produce.
...
**Waitlist (demand-sensing):** Instead of guessing which future track to build, visitors and users vote with their email. `waitlist_entries` table (unique on `(email, topic)` so re-submitting is a no-op), `WaitlistController`, topics defined in `config/waitlist.php` (currently `observability`, `ai-for-devs`, `ai-for-support`). Adding a candidate track = one config entry + a card. Frontend: `WaitlistSection.vue` (homepage "coming soon" cards with inline email capture), `WaitlistVote.vue` (client dashboard right rail), `WaitlistAdminCard.vue` (admin dashboard — per-track counts with bars). The track that pulls hardest is the one to build next. Tests: `tests/Feature/Api/WaitlistApiTest.php`.

**CI (GitHub Actions):** `.github/workflows/ci.yml` runs on push to `main` and on PRs, with per-ref concurrency cancellation. Job 1 (backend): PHP 8.4, `pint --test` (style must be clean — run `ddev exec vendor/bin/pint --dirty` before committing), then `php artisan test` on sqlite in-memory. Job 2 (frontend): Node 24, `npm ci` + `npm run build` with a placeholder `NUXT_PUBLIC_API_BASE`. There is no typecheck or frontend test step yet (planned — see `docs/roadmap.md`).

**Error monitoring (Sentry):** Wired on both sides but dormant until a DSN is set. Backend: `sentry/sentry-laravel`, `config/sentry.php`, integration registered in `bootstrap/app.php`; env `SENTRY_LARAVEL_DSN`, `SENTRY_TRACES_SAMPLE_RATE`, `SENTRY_RELEASE` (set to the git SHA per deploy). Frontend: `frontend/sentry.client.config.ts`, env `NUXT_PUBLIC_SENTRY_DSN`. No DSN = no-op, so local dev is unaffected.

**Deployment:** `DEPLOYMENT.md` (repo root) is the production runbook — two stateless containers (API: root `Dockerfile`, php-fpm + nginx on 8080; frontend: `frontend/Dockerfile`, Nitro Node on 3000) + managed MySQL and Redis. The API entrypoint (`docker/entrypoint.sh`) links storage, caches config, and runs migrations on boot. Health check at `/up`. Read the runbook before touching anything deploy-related.
