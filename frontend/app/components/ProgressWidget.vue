<template>
  <div class="rounded-2xl border border-gray-200/80 bg-white dark:border-neutral-800 dark:bg-neutral-900">
    <div class="border-b border-gray-100 px-5 py-4 dark:border-neutral-800">
      ...
      <p class="mt-0.5 text-xs text-gray-500 dark:text-neutral-400">XP, streak, and badges</p>
    </div>

    <div class="p-5">
      <!-- Loading skeleton -->
      <div v-if="loading" class="space-y-3">
        <div class="h-8 animate-pulse rounded-md bg-gray-200 dark:bg-neutral-800" />
        <div class="h-8 animate-pulse rounded-md bg-gray-200 dark:bg-neutral-800" />
      </div>
      ...
      <template v-else-if="progress">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="rounded-xl border border-gray-100 p-3 text-center dark:border-neutral-800">
            <p class="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
              {{ progress.xp_points }}
            </p>
            <p class="mt-0.5 text-xs text-gray-500 dark:text-neutral-400">XP earned</p>
          </div>
          <div class="rounded-xl border border-gray-100 p-3 text-center dark:border-neutral-800">
        ...
        <div v-if="progress.badges.length" class="mt-4">
          <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-neutral-500">Badges</p>
            ...
            <UTooltip v-for="badge in progress.badges" :key="badge.key" :text="`${badge.name} — ${badge.description}`">
              <span
                class="inline-flex items-center gap-1.5 rounded-full border border-gray-200 px-2.5 py-1 text-xs
                       font-medium text-gray-700 dark:border-neutral-700 dark:text-neutral-300"
              >
        ...
        <p v-else class="mt-4 text-xs text-gray-500 dark:text-neutral-400">
        ...
        <div class="mt-4 border-t border-gray-100 pt-4 dark:border-neutral-800">
          ...
          <div v-if="isPublic && publicSlug" class="mt-3 flex items-center gap-2">
            <code class="flex-1 truncate rounded-md bg-gray-50 px-2.5 py-1.5 text-xs text-gray-600 dark:bg-neutral-950 dark:text-neutral-300">
              {{ profileUrl }}
            </code>
            <UButton
              size="xs"
